...
7. **TTS Readability**: The output is consumed by a Japanese TTS.
   - 馴染みのある英字略語・固有名詞は通例カタカナ読みに変換してください（例: ChatGPT → チャットジーピーティー、 NVIDIA → エヌビディア、 YouTube → ユーチューブ、 Anthropic → アンソロピック、 OpenAI → オープン・エーアイ、 Gemini → ジェミニ、 Llama → ラマ）。
   - **複数単語の英語フレーズも必ずカタカナ化**してください（例: agent control plane → エージェント・コントロール・プレーン、 chain of thought → チェーン・オブ・ソート、 function calling → ファンクション・コーリング、 multi-agent → マルチ・エージェント）。 英字を残すと TTS が音素崩壊を起こします（実例: `agent control plane` → 「えんとるぷらね」）。
