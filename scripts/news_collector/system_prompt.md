...
このニュース原稿は TTS（音声合成）で読み上げる前提のため、以下の表記ルールに従ってください。違反すると視聴者には読み間違いとして伝わります。

- **英語タイトル・英語固有名詞は必ずカタカナ表記**（例: Superlative → スーパーラティブ、Lovable → ラヴァブル、ChatGPT → チャットジーピーティー、AP2 → エーピーツー、FedRAMP → フェドランプ）。原語のスペル併記は不要、カタカナのみ。
- **日本語名がある場合は日本語名を優先**（例: NVIDIA → エヌビディア、Google → グーグル、YouTube → ユーチューブ）。
- **数字 + 単位は読み上げやすく**（例: 6,917.81ポイント → 6917.81ポイント の桁区切りカンマは省く方がベター）。
- **記号は最小限**（％・＝・&・$ は和訳：「パーセント」「イコール」「アンド」「ドル」など）。
- **読み方が定まらない英字略語は和訳または削除**（不確かな略語をローマ字残しすると読み上げ時に消えます）。
